...
import { Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, styled, TablePagination } from '@mui/material';
...
const ManageUserTable: React.FC<ManageUserTableProps> = ({ data  }) => {
 ...
 <UpdateRoleModal
    ...
    <TableContainer sx={{ maxHeight: 440 }} component={Paper}>
        ...
        <input value={search}  onChange={(e) => setSearch(e.target.value)} className='ms-5' placeholder='search' type="text"/>
        ...
        <TableBody>
          ...
          .filter((user: User) => {
            ...
            >
<TableCell    align="center"><div><Pencil ></Pencil><Trash className='clickable' onClick={()=>{setUserToDelete((item as User).userId) ;handleClickOpen()}}></Trash></div></TableCell>

<TableCell    component="th" scope="row">
                {(item as User).userFirstName} {(item as User).userLastName}
              </TableCell>
              <TableCell    align="center">{(item as User).userEmail}</TableCell>
              <TableCell    align="center">{(item as User).userRole}</TableCell>

              <TableCell  align="center"  ><input  onClick={() => UpdateRole((item as User).userId)} type='button' value={"update Role"}></input></TableCell>
     ...
     />
      ...
      >
        ...
        <DialogContent>
          <DialogContentText className='text-center bg-danger rounded-3 text-light' id="alert-dialog-description"><b>
          {userToDelete !=0 ?data.filter(user => user.userId==userToDelete)[0].userFirstName : ""} {userToDelete !=0 ?data.filter(user => user.userId==userToDelete)[0].userLastName:""}
          </b></DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleClose}>Cancel</Button>
          <Button className='text-danger' onClick={()=>deleteUser(userToDelete)} autoFocus>
            Delete any way
          </Button>
        </DialogActions>
      </Dialog>
